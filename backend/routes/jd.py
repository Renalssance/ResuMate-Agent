...
async def create_jd(request: JDCreateRequest, current_user: User = Depends(get_current_user)):
    ...
    logger.info(
        "JD create start | user_id=%s title=%s company=%s chars=%s",
        current_user.id,
        request.title,
        request.company,
        len(jd_text),
    ...
    # 使用 LLM 解析
    from backend.agent.interview_tools import _get_llm, _invoke_llm, _parse_llm_json, JD_PARSE_PROMPT
    ...
    if llm:
        try:
            ...
            resp_content = _invoke_llm(
                ...
                {"user_id": current_user.id, "title": title, "company": company, "jd_text_chars": len(jd_text)},
            )
            structured_data = _parse_llm_json(resp_content)
            logger.info("JD structured parsing completed | user_id=%s parsed=%s", current_user.id, bool(structured_data))
            if structured_data:
    ...
    if not llm:
        logger.warning("Skip JD structured parsing because LLM is unavailable | user_id=%s", current_user.id)
    ...
    try:
        ...
        db.add(jd)
        db.flush()
        logger.info("JD database row created | user_id=%s jd_id=%s title=%s", current_user.id, jd.id, title)
        try:
            vector_store.upsert_jd(
                user_id=current_user.id,
                jd_id=jd.id,
                title=title,
                company=company,
            ...
            logger.info("JD vector upsert completed | user_id=%s jd_id=%s title=%s", current_user.id, jd.id, title)
        except Exception as e:
            logger.exception("JD 向量入库失败: %s", e)
            raise HTTPException(status_code=500, detail=f"JD 向量入库失败: {e}")
        db.commit()
        db.refresh(jd)
        logger.info("JD create completed | user_id=%s jd_id=%s title=%s", current_user.id, jd.id, title)
        return JDCreateResponse(id=jd.id, title=title, company=company, message="JD 创建、解析并写入向量库成功")
    except Exception as e:
        db.rollback()
...
async def delete_jd(jd_id: int, current_user: User = Depends(get_current_user)):
    ...
    try:
        ...
        if not jd:
            raise HTTPException(status_code=404, detail="JD 不存在")
        try:
            vector_store.delete_profile(doc_type="jd", user_id=current_user.id, source_id=jd.id)
        except Exception as e:
            logger.warning("删除 JD 向量失败，继续删除数据库记录: %s", e)
        db.delete(jd)
        db.commit()
