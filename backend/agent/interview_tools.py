...
from backend.logging_config import log_llm_error, log_llm_prompt, log_llm_response
...
def _get_llm(temperature: float = 0.1):
    ...
    if not API_KEY or not MODEL:
        logger.warning("LLM is not configured: ARK_API_KEY or MODEL is missing")
        return None
    logger.info("Creating LLM client | model=%s base_url=%s temperature=%s", MODEL, BASE_URL, temperature)
...
def _invoke_llm(llm, prompt: str, task: str, metadata: dict | None = None) -> str:
    metadata = dict(metadata or {})
    metadata.update({"model": MODEL, "prompt_chars": len(prompt)})
    logger.info("LLM call start | task=%s prompt_chars=%s", task, len(prompt))
    log_llm_prompt(task, prompt, metadata)
    ...
    try:
        ...
        logger.exception("LLM call failed | task=%s elapsed_ms=%s", task, elapsed_ms)
        raise

    elapsed_ms = int((time.perf_counter() - start_time) * 1000)
    metadata.update({"elapsed_ms": elapsed_ms, "response_chars": len(content)})
    log_llm_response(task, content, metadata)
    logger.info(
        "LLM call end | task=%s response_chars=%s elapsed_ms=%s",
        task,
        len(content),
        elapsed_ms,
    )
    ...
    def analyze_resume(raw_text: str, filename: str = "resume") -> str:
        ...
        try:
            ...
            content = _invoke_llm(
                ...
                prompt,
                "tool.analyze_resume",
                {"user_id": user_id, "filename": filename, "raw_text_chars": len(raw_text)},
            )
    ...
    def analyze_jd(jd_text: str, title: str = "", company: str = "") -> str:
        ...
        try:
            ...
            content = _invoke_llm(
                ...
                prompt,
                "tool.analyze_jd",
                {"user_id": user_id, "title": title, "company": company, "jd_text_chars": len(jd_text)},
            )
            structured = _parse_llm_json(content)
        ...
        if mode == "question":
            try:
                ...
                content = _invoke_llm(
                    llm,
                    prompt,
                    "tool.mock_interview.question",
                    {"user_id": user_id, "jd_id": jd_id, "interview_type": interview_type},
                )
                result = _parse_llm_json(content)
